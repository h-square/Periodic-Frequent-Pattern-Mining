            ...
            for ( const auto& pair : items_ordered_by_frequency ) {
                ...
                if ( std::find( transaction.cbegin(), transaction.cend(), item ) != transaction.cend() ) {
                    ...
                    if ( it == curr_fpnode->children.cend() ) {
                        // the child doesn't exist, create a new node
                        const auto curr_fpnode_new_child = std::make_shared<FPNode>( item, curr_fpnode );
                        curr_fpnode_new_child->tid_list.insert(tids[i]);
                    ...
                    auto curr_fpnode_child = *it;
                    ++curr_fpnode_child->frequency;
                    curr_fpnode_child->tid_list.insert(tids[i]);
...
int main(int argc, char* argv[]){
    ...
    set<onlyPattern> serial_patterns;
    if(flag == 1)
    {
	    for(int i = 0; i < max_runs_serial; i++) {
            ...
            serial_patterns = fptree_growth( fptree );
    	    ////////////////////////////////////////////////////////////
    	    get_walltime(&end);
		    time = end - start;
		    ts += time;
            pfpattern_serial = serial_patterns.size();
	    }
    ...
    /////////////////////////////////////////////////////////////////////////////////////////////////////
    cout << "#PFPs: " << pfpattern_serial << endl;
    /////////////////////////////////////////////////////////////////////////////////////////////////////
    vector<int> thds;
    vector<double> tps;
    vector<double> sus;
	...
	for(int p = start_thread; p <= max_threads; p = p + 1) {
		...
		for(int i = 0; i < max_runs_parallel; i++) {
			...
			get_walltime(&end);
            cout<<"Parallel PFPs "<<patterns.size();
            int not_found = 0;
            for(auto pattern:patterns) {
                if(serial_patterns.find(pattern) == serial_patterns.end()) {
                    not_found++;
                }
            }
            cout<<"Mismatched patterns: "<<not_found<<endl;
            //////////////////////////////////////////////////////////////////////////////////////////////
